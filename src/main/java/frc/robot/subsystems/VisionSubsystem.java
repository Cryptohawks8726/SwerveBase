...
import frc.robot.util.LightweightAprilTagLayout;
import frc.robot.util.LimelightHelpers;
import frc.robot.startup.RobotContainer;
...
public class VisionSubsystem extends SubsystemBase {
    // TODO: Experiment with kD and higher kP
    ...
    private final Pose2d[] scoringSetpoints = new Pose2d[] {
            VisionConstants.nearGoalAimingPosition,
    };
    ...
    private final LightweightAprilTagLayout aprilTagLayout = VisionConstants.layout;

    private final Field2d field = new Field2d();

    private final Pigeon2 johnGyro = new Pigeon2(VisionConstants.gyroID);

    public VisionSubsystem(SwerveSubsystem newSwerve) {
        LimelightHelpers.setCameraPose_RobotSpace("limelight-april",
                VisionConstants.forwardOffset,
                VisionConstants.sideOffset,
                VisionConstants.upOffset,
                VisionConstants.rollOffset,
                VisionConstants.pitchOffset,
                VisionConstants.yawOffset);

        LimelightHelpers.SetFiducialIDFiltersOverride("limelight-april", aprilTagLayout.getIDs());
    ...
    /**
     * Outputs a tag-relative swerve driveToPose8726 command
     * 
     * @param desiredDistance The setpoint pose of the robot relative to the given
     *                        apriltag
     * @param tagID           The ID of the apriltag which desiredDistance is
     *                        relative to
     */
    public Command driveToTagPose(Supplier<Pose2d> desiredDistance, int tagID) {
        Pose2d desiredPose = desiredDistance.get();

        trackingTag = true;

        // Uses the most recent tag for creating setpoints
        Pose2d tagPose = aprilTagLayout.getTagPose(tagID).get();
        ...
        simulateTagRelativeSetpoint(desiredPose, tagID);

        return RobotContainer.swerve.driveToPose8726(targetPose);
    }
